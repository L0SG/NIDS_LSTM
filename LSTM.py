...
class ADInput(object):
    ...
    def __init__(self, config, data, name=None):
        ...
        self.input_data = []
        self.targets = []
        self.input_data, self.targets = encoding.input_producer(data, batch_size, num_steps, name=name)
        """
        for idx in xrange(0, INPUT_SIZE):
            input_data_temp, targets_temp = encoding.input_producer(
                    data[idx], batch_size, num_steps, name=name)
            self.input_data.append(input_data_temp)
            self.targets.append(targets_temp)
        """

class ADModel(object):
    """The AD model."""
    def __init__(self, is_training, config, input_, vocab):
        self._input = input_
        ...
        def lstm_cell():
            return tf.nn.rnn_cell.BasicLSTMCell(size*8 + 5, forget_bias=0.0, state_is_tuple=True)
        attn_cell = lstm_cell
        ...
        with tf.device("/gpu:0"):
            ...
            for i in range(INPUT_SIZE):
                embedding.append([])
                if i in encoding.idx_continuous:
        ...
        output = tf.reshape(tf.concat(1,outputs), [-1, size*8 + 5])
        softmax_w =[]
        softmax_b =[]
        logits = []
        ...
        for i in range(INPUT_SIZE):
            softmax_w.append([])
            softmax_b.append([])
            if i == 0 or i ==1 or i==3 or i==6 or i==8:
                softmax_w[i] = tf.get_variable(
                    "softmax_w" + str(i), [size*8+5, 1], dtype=data_type())
                softmax_b[i] = tf.get_variable("softmax_b"+str(i), [1], dtype=data_type())
            else:
                softmax_w[i] = tf.get_variable(
                    "softmax_w" + str(i), [size*8+5, size], dtype=data_type())
                softmax_b[i] = tf.get_variable("softmax_b"+str(i), [size], dtype=data_type())
            logit = tf.matmul(output, softmax_w[i]) + softmax_b[i]
            print(logit)
            # test: not correct
            logit = logit[:,0]
            print(logit)
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope("Test"):
            ...
            with tf.variable_scope("Model", reuse=True, initializer=initializer):
                mtest = ADModel(is_training=False, config=eval_config, input_=test_input, vocab=vocabulary)
